...
%% Figrue4
mycolor = [255 0 0; 153 51 255; 237 176 33;...    %BCC-CSM2-MR, CanESM5, CESM2-WACCM
...
figure 
set(gcf,'position',[500,200,520,496])
Cnet_PDF= tight_subplot(1,1,[0.01 0.02],[0.65 0.03],[0.16 0.06])
hold on

h26_Cnet = raincloud_plot(reC26_gbUN,'box_on', 0,...
                     'color', [0.36,0.92,0.82], 'cloud_edge_col', [0.36,0.92,0.82],...
                     'bandwidth', 12,'density_type', 'ks','alpha', 0.5);                 
set(h26_Cnet{2},'Marker','none')

h45_Cnet = raincloud_plot(reC45_gbUN,'box_on', 0, ...
    'color', [0.80,1.00,0.47], 'cloud_edge_col', [0.56,0.80,0.16],...
    'bandwidth', 12,'density_type', 'ks','alpha', 0.5);
set(h45_Cnet{2},'Marker','none')

h85_Cnet = raincloud_plot(reC85_gbUN,'box_on', 0,...
    'color', [0.97,0.63,0.59], 'cloud_edge_col', [1.00,0.47,0.41], ...
    'bandwidth', 12,'density_type', 'ks','alpha', 0.2);
set(h85_Cnet{2},'Marker','none')

set(gca,'YTickLabelMode','auto');
set(gca,'XTickLabelMode','auto');
set(gca, 'YLim',[0 0.015],'XLim',[-150 300]);
set(gca,'Fontname','Arial','FontSize',11)
set(gca,'linewidth',1,'box','on')
text(56,0.0127,'SSP1-2.6','Fontname','Arial','FontSize',11,'color',[0.36,0.92,0.82])
text(90,0.0088,'SSP2-4.5','Fontname','Arial','FontSize',11,'color',[0.56,0.80,0.16])
text(150,0.005,'SSP5-8.5','Fontname','Arial','FontSize',11,'color',[1.00,0.47,0.41])
xlabel('\DeltaCland (Pg C)','Fontname','Arial','FontSize',12)
ylabel('PDF','Fontname','Arial','FontSize',12)

Cnet_fcts= tight_subplot(1,1,[0.01 0.02],[0.18 0.46],[0.16 0.06])
% ssp126
reNPP_tauE0_f26 = 26.5;
NPP0_reTauE_f26 = 27.4;
reNPPtauE_f26 =9.0;
reXp_f26 = 37.1;

% ssp245
reNPP_tauE0_f45 = 36.0;
NPP0_reTauE_f45 = 32.2;
reNPPtauE_f45 = 13.1;
reXp_f45 = 18.7;

% ssp585
reNPP_tauE0_f85 = 45.3;
NPP0_reTauE_f85 = 23.8;
reNPPtauE_f85 = 13.7;
reXp_f85 = 17.2;

SSPs_Y= [3 2 1];
Cnet_fcts = [reNPP_tauE0_f26 NPP0_reTauE_f26 reNPPtauE_f26 reXp_f26; ...
             reNPP_tauE0_f45 NPP0_reTauE_f45 reNPPtauE_f45 reXp_f45; ...
             reNPP_tauE0_f85 NPP0_reTauE_f85 reNPPtauE_f85 reXp_f85]
ssp3_bar_fcts = barh(SSPs_Y,Cnet_fcts,'stacked')
ssp3_bar_fcts(1).FaceColor = [0.47 0.67 0.19]
ssp3_bar_fcts(2).FaceColor = [ 0.93 0.69 0.13]
ssp3_bar_fcts(3).FaceColor = [0.72 0.72 0.87]
ssp3_bar_fcts(4).FaceColor = [0  0 1]
ssp3_bar_fcts(1).EdgeColor = [1 1 1]
ssp3_bar_fcts(2).EdgeColor = [1 1 1]
ssp3_bar_fcts(3).EdgeColor = [1 1 1]
ssp3_bar_fcts(4).EdgeColor = [1 1 1]

set(gca, 'YLim',[0.2 3.8],'XLim',[0 100]);
set(gca,'Fontname','Arial','FontSize',11);
set(gca,'linewidth',1,'box','on')
yticks([1 2 3]);
yticklabels({'SSP5-8.5','SSP2-4.5','SSP1-2.6'});
xlabel('Contribution to model spread in \DeltaCland (%)')
legSSP3 = legend({'\DeltaNPP X \tau_E_0 ','NPP_0 X \Delta\tau_E ','\DeltaNPP X \Delta\tau_E ','\DeltaXp'});
set(legSSP3, 'interpreter', 'tex')
set(legSSP3,'NumColumns',4,'Position',[0.14,0.019,0.84,0.047],'color','none','EdgeColor','k','Fontname','Arial','Fontsize',9)
